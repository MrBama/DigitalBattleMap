...
{
    public static Cursor CreateCursor(Brush brush, int size)
    {
        return CreateCursor(brush, new Pen(), size);
    }

    public static Cursor CreateCursor(Brush brush, Pen pen, int size)
    {
        var drawingVisual = new DrawingVisual();
        using (var drawingContext = drawingVisual.RenderOpen())
        {
            var fillSize = (int)Math.Max(size - pen.Thickness * 2, 1);
            //drawingContext.DrawRectangle(brush, new Pen(), new Rect(0, 0, size, size));
